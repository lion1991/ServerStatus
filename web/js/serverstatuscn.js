...
function uptime() {
	$.getJSON("json/statscn.json", function(result) {
		...
		for (var i = 0, rlen=result.servers.length; i < rlen; i++) {
			...
			if (!result.servers[i].online4 && !result.servers[i].online6) {
				...
				ExpandRow[0].children["expand_ping"].innerHTML = "Hostdare/香港落地/深圳入口/台湾Hinet: " + result.servers[i].time_hd + "ms / " + result.servers[i].time_ld + "ms / " + result.servers[i].time_sz + "ms / " + result.servers[i].time_tw + "ms"

                // ping
                var PING_hd = result.servers[i].ping_hd.toFixed(0);
                var PING_ld = result.servers[i].ping_ld.toFixed(0);
                var PING_sz = result.servers[i].ping_sz.toFixed(0);
                var PING_tw = result.servers[i].ping_tw.toFixed(0);
                if (PING_hd >= 20 || PING_ld >= 20 || PING_sz >= 20 || PING_tw >= 20)
                    TableRow.children["ping"].children[0].children[0].className = "progress-bar progress-bar-danger";
                else
                    TableRow.children["ping"].children[0].children[0].className = "progress-bar progress-bar-success";
	            TableRow.children["ping"].children[0].children[0].innerHTML = PING_hd + "%💻" + PING_ld + "%💻"  + PING_sz + "%💻" + PING_tw + "%";
