...
int CMain::HandleMessage(int ClientNetID, char *pMessage)
{
	...
	if(str_comp_num(pMessage, "update", sizeof("update")-1) == 0)
	{
		...
		if(rStart["ping_hd"].type)
			pClient->m_Stats.m_ping_hd = rStart["ping_hd"].u.dbl;
		if(rStart["ping_ld"].type)
			pClient->m_Stats.m_ping_ld = rStart["ping_ld"].u.dbl;
		if(rStart["ping_sz"].type)
			pClient->m_Stats.m_ping_sz = rStart["ping_sz"].u.dbl;
		if(rStart["ping_tw"].type)
			pClient->m_Stats.m_ping_tw = rStart["ping_tw"].u.dbl;
	    if(rStart["time_hd"].type)
			pClient->m_Stats.m_time_hd = rStart["time_hd"].u.integer;
		if(rStart["time_ld"].type)
			pClient->m_Stats.m_time_ld = rStart["time_ld"].u.integer;
		if(rStart["time_sz"].type)
			pClient->m_Stats.m_time_sz = rStart["time_sz"].u.integer;
		if(rStart["time_tw"].type)
			pClient->m_Stats.m_time_tw = rStart["time_tw"].u.integer;
		if(rStart["tcp"].type)
			...
			if(pClients[i].m_Connected)
			{
				...
				str_format(pBuf, sizeof(aFileBuf) - (pBuf - aFileBuf),
				 "{ \"name\": \"%s\",\"type\": \"%s\",\"host\": \"%s\",\"location\": \"%s\",\"online4\": %s, \"online6\": %s,\"ip_status\": %s,\"uptime\": \"%s\",\"load_1\": %.2f, \"load_5\": %.2f, \"load_15\": %.2f,\"ping_hd\": %.2f, \"ping_ld\": %.2f, \"ping_sz\": %.2f, \"ping_tw\": %.2f,\"time_hd\": %" PRId64 ", \"time_ld\": %" PRId64 ", \"time_sz\": %" PRId64 ", \"time_tw\": %" PRId64 ", \"tcp_count\": %" PRId64 ", \"udp_count\": %" PRId64 ", \"process_count\": %" PRId64 ", \"thread_count\": %" PRId64 ", \"network_rx\": %" PRId64 ", \"network_tx\": %" PRId64 ", \"network_in\": %" PRId64 ", \"network_out\": %" PRId64 ", \"cpu\": %d, \"memory_total\": %" PRId64 ", \"memory_used\": %" PRId64 ", \"swap_total\": %" PRId64 ", \"swap_used\": %" PRId64 ", \"hdd_total\": %" PRId64 ", \"hdd_used\": %" PRId64 ", \"custom\": \"%s\" },\n",
					...
					aUptime, pClients[i].m_Stats.m_Load_1, pClients[i].m_Stats.m_Load_5, pClients[i].m_Stats.m_Load_15, pClients[i].m_Stats.m_ping_hd, pClients[i].m_Stats.m_ping_ld, pClients[i].m_Stats.m_ping_sz, pClients[i].m_Stats.m_ping_tw, pClients[i].m_Stats.m_time_hd, pClients[i].m_Stats.m_time_ld, pClients[i].m_Stats.m_time_sz, pClients[i].m_Stats.m_time_tw,pClients[i].m_Stats.m_tcpCount,pClients[i].m_Stats.m_udpCount,pClients[i].m_Stats.m_processCount,pClients[i].m_Stats.m_threadCount,pClients[i].m_Stats.m_NetworkRx, pClients[i].m_Stats.m_NetworkTx, pClients[i].m_Stats.m_NetworkIN, pClients[i].m_Stats.m_NetworkOUT, (int)pClients[i].m_Stats.m_CPU, pClients[i].m_Stats.m_MemTotal, pClients[i].m_Stats.m_MemUsed, pClients[i].m_Stats.m_SwapTotal, pClients[i].m_Stats.m_SwapUsed, pClients[i].m_Stats.m_HDDTotal, pClients[i].m_Stats.m_HDDUsed, pClients[i].m_Stats.m_aCustom);
				pBuf += strlen(pBuf);
			}
			else
			{
